...
class CambrianQwenConfig(Qwen2Config):
    ...
    # Enable SSM memory fusion when True and an `ssm_compressor` is passed to model forward
...
class CambrianQwenModel(CambrianMetaModel, Qwen2Model):
    ...
    def _decoder_layer_forward_ssm(
        self,
        decoder_layer,
        hidden_states,
        attention_mask,
        ...
        # Mirror Qwen2 decoder layer structure and insert SSM fusion after self-attn.
        ...
        if ssm_compressor is not None and ssm_compressor.has_memory(layer_idx):
            hidden_states = ssm_compressor.fuse(
                layer_idx=layer_idx,
                hidden_states=pre_attn_hidden,
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            elif getattr(self.config, 'ssm_enabled', False) and ssm_compressor is not None:
                layer_outputs = self._decoder_layer_forward_ssm(
                    decoder_layer,
                    hidden_states,
                    attention_mask,
                    position_ids,
...
class CambrianQwenForCausalLM(Qwen2ForCausalLM, CambrianMetaForCausalLM):
    ...
    def forward(
        ...
        # Avoid sending nn.Module via Trainer inputs; prefer model-attached compressor.
